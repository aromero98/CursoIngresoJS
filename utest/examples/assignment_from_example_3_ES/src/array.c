...
{
    int i,retorno = -1;
    if(pEmployee != NULL && length > 0) //que el empleado sea valido y mayor a cero
    {
        for(i=0; i<length; i++) //recorre el array
...
int addEmployee(employee* pEmployee, int length, int id, char name[],char lastName[],float salary,int sector)
{
    ...
    if(pEmployee != NULL && length > 0) //que el empleado sea valido y mayor a cero
    {
        ...
        for(i=0; i<length; i++)//recorro el array para encontrar una posicion libre
        {
            if(pEmployee[i].isEmpty==1) //1=ESTA libre 0=ESTA ocupado
            {
 ...
 *
 */
employee* findEmployeeById(employee* pEmployee, int length,int id)//busca posicion del empleado en la memoria
{
    int i;
